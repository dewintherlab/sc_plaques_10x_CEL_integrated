...
#======================================================================
## Setup
dir.create("mosaicY_myeloid_cells", showWarnings = F)
...
egALL      <- row.names(final.pop.call.from_full.integrated.mye.seurat@assays$RNA@counts)
egALL.expr <- GetAssayData(final.pop.call.from_full.integrated.mye.seurat, slot = "data", assay = "RNA")[egALL,]
...
egY.expr    <- GetAssayData(final.pop.call.from_full.integrated.mye.seurat, slot = "data", assay = "RNA")[egY,]
...
# Get male patients
male.cells <- row.names(final.pop.call.from_full.integrated.mye.seurat@meta.data[final.pop.call.from_full.integrated.mye.seurat@meta.data$Sex == "male",])
...
ggsave("mosaicY_myeloid_cells/chrY_genes_per_patient_per_cell.boxplot.pdf")
...
ggbarplot(m.egY.patient.ratio, x = "Patient", y = "loss.ratio", title = "chrY genes ratio of cells with 0 reads on Y per (male) patient", xlab = "Patient", ylab = "Loss Ratio") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
ggsave("mosaicY_myeloid_cells/chrY_genes_ratio_per_patient.barplot.pdf")
...
avg.exp.all_but_Y <- as.data.frame(GetAssayData(final.pop.call.from_full.integrated.mye.seurat, slot = "data", assay = "RNA")[,unlist(egY.zero.cells)])
avg.exp.all_but_Y <- avg.exp.all_but_Y[which(!row.names(avg.exp.all_but_Y) %in% egY),]
pdf("mosaicY_myeloid_cells/not_on_chrY_Genes_avgexp_in_no_chrY_cells.hist.pdf")
...
VlnPlot(object = subset(final.pop.call.from_full.integrated.mye.seurat, cells = unlist(egY.zero.cells)), group.by = "Patient", features = c("nFeature_RNA","nCount_RNA"), y.max = 15000) + 
  theme_pubr(base_size = 14, x.text.angle = 45, legend = "right") +
  theme(panel.background = element_blank(),
        title            = element_text(size = 16, face = "bold"), 
        aspect.ratio     = 1
  )
ggsave("mosaicY_myeloid_cells/featureCount.chrY_Zero cells.pdf")

# Plot the average expression in nonzero Y cells
avg.exp.nonzeroY <- as.data.frame(GetAssayData(final.pop.call.from_full.integrated.mye.seurat, slot = "data", assay = "RNA")[,unlist(egY.notzero.cells)])
avg.exp.nonzeroY <- avg.exp.nonzeroY[which(!row.names(avg.exp.nonzeroY) %in% egY),]
pdf("mosaicY_myeloid_cells/not_on_chrY_Genes_avgexp_in_notzero_chrY_cells.hist.pdf")
...
VlnPlot(object = subset(final.pop.call.from_full.integrated.mye.seurat, cells = unlist(egY.notzero.cells)), group.by = "Patient", features = c("nFeature_RNA","nCount_RNA"), y.max = 15000) + 
  theme_pubr(base_size = 14, x.text.angle = 45, legend = "right") +
  theme(panel.background = element_blank(),
        title            = element_text(size = 16, face = "bold"), 
        aspect.ratio     = 1
  )
ggsave("mosaicY_myeloid_cells/featureCount.chrY_notZero cells.pdf")
...
ggbarplot(m.egY.cells.abs[m.egY.cells.abs$Patient %in% c("P1", "P2", "P3"),], x = "Patient", y = "cell.count", fill = "chrY.genes", title = "Cells with or without chrY gene expression", xlab = "Patient", ylab = "Cell numbers", palette = "Paired", position = position_dodge2()) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
ggsave("mosaicY_myeloid_cells/chrY_genes_ratio_per_patient.10X.barplot.pdf")

ggbarplot(m.egY.cells.abs[!m.egY.cells.abs$Patient %in% c("P1", "P2", "P3"),], x = "Patient", y = "cell.count", fill = "chrY.genes", title = "Cells with or without chrY gene expression", xlab = "Patient", ylab = "Cell numbers", palette = "Paired", position = position_dodge2()) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
ggsave("mosaicY_myeloid_cells/chrY_genes_ratio_per_patient.CS.barplot.pdf")
...
for(n in 1:1000){
  ...
  # Retrieve expression data per cell
  tmp.egNOTY.expr <- as.data.frame(GetAssayData(final.pop.call.from_full.integrated.mye.seurat, slot = "data", assay = "RNA")[CTRL.boots.genes[[n]],])
...
ggsave("mosaicY_myeloid_cells/genes_ratio_per_patient_bootstrapped.barplot.pdf")
...
pdf("mosaicY_myeloid_cells/bootstrap_fit_cutoff.scatter.pdf")
...
ggsave("mosaicY_myeloid_cells/bootstrap_directionality.barplot.pdf")
...
ggsave("mosaicY_myeloid_cells/genes_ratio_diff_per_patient_bootstrapped.barplot.pdf")
