...
##========================================================================================================================
dir.create("full_set_archetype_metadata_correlations/Individual trait UMAPs", showWarnings = F, recursive = T)
dir.create("full_set_archetype_metadata_correlations/Individual trait distribution", showWarnings = F, recursive = T)
dir.create("full_set_archetype_metadata_correlations/Individual trait UMAPs binned", showWarnings = F, recursive = T)
dir.create("full_set_archetype_metadata_correlations/Individual trait distribution binned", showWarnings = F, recursive = T)
...
for(theTrait in colnames(meta.data)[-2]){
  ...
  customUMAP(object    = full.43p.seurat, 
             ...
             title     = theTrait,
             shuffle   = T, 
             file.name = paste("full_set_archetype_metadata_correlations/Individual trait UMAPs/UMAP ", theTrait, ".pdf"))
}
...
for(theTrait in colnames(meta.data[-2])){
  ...
  trait.dist[[theTrait]]            <- data.frame(table(full.43p.seurat@meta.data[,theTrait]))
  ...
  for(thePop in unique(full.43p.seurat$celltypes.43p)){
    cat(paste("\tAdding distribution for pop: ", thePop, "...\n", sep =""))
    
    # Subset out the desired population and get the distribution
    tmp.pop.seurat        <- subset(full.43p.seurat, celltypes.43p == thePop)
...
padj.sig.trait.dist <- apply(sig.trait.dist, c(1,2), function(x){p.adjust(p = x, method = "BH", n = length(unique(full.43p.seurat$celltypes.43p)))})
...
for(theTrait in names(trait.dist)){
  ...
  m$Population <- factor(m$Population, levels = c("Total",unique(full.43p.seurat$celltypes.43p)))
  levels(m$Population)
  ...
  if(theTrait %in% row.names(padj.sig.trait.dist[rowSums(padj.sig.trait.dist <= 0.05) > 0,])){ # If there are sig diff populations, plot significance
    ...
    ggplot(m, aes(x = Population, y = Frequency)) + 
      ...
      theme(axis.text.y  = element_text(angle = 0, size = 14),
            ...
            legend.position = "top",
            aspect.ratio = 1.2)
    ggsave(paste("full_set_archetype_metadata_correlations/Individual trait distribution/", theTrait, ".pdf", sep = ""), width = label_width + 4, height = 6)
...
for(theTrait in colnames(binned.meta.data)[-2]){
  ...
  customUMAP(object    = full.43p.seurat, 
             ...
             title     = theTrait,
             shuffle   = T, 
             file.name = paste("full_set_archetype_metadata_correlations/Individual trait UMAPs binned/UMAP ", theTrait, ".pdf"))
}
  ...
  if(theTrait %in% row.names(padj.sig.trait.dist[rowSums(padj.sig.trait.dist <= 0.05) > 0,])){ # If there are sig diff populations, plot significance
    ...
    ggplot(m, aes(x = Population, y = Frequency)) + 
      ...
      theme(axis.text.y  = element_text(angle = 0, size = 14),
            ...
            legend.position = "top",
            aspect.ratio = 1.2)
    ggsave(paste("full_set_archetype_metadata_correlations/Individual trait distribution binned/", theTrait, ".pdf", sep = ""), width = label_width + 4, height = 6)
