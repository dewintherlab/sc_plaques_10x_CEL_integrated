...
#======================================================================
## Setup
dir.create("mosaicY", showWarnings = F)
...
## Keep only genes expressed in the immune cells
# Use immune cells only
immune.idents <- as.vector(unique(Idents(integrated.full.seurat))[grep("Endo|Muscle" , unique(Idents(integrated.full.seurat)), invert = T)])
immune.seurat <- subset(integrated.full.seurat, idents = immune.idents)
customUMAP(object = immune.seurat, group.by = "archetype", file.name = "mosaicY/immune.UMAP.pdf", pt.size = 1.4, plot.width = 15, legend.pos = "right", cols = full_set.arch.colors)
rm(integrated.full.seurat)

# Get the immune expressed gene set
egALL      <- row.names(immune.seurat@assays$RNA@counts)
egALL.expr <- GetAssayData(immune.seurat, slot = "data", assay = "RNA")[egALL,]
...
egY.expr    <- GetAssayData(immune.seurat, slot = "data", assay = "RNA")[egY,]
...
# Get male patients
male.cells <- row.names(immune.seurat@meta.data[immune.seurat@meta.data$Sex == "male",])
...
ggsave("mosaicY/chrY_genes_per_patient_per_cell.boxplot.pdf")
...
ggbarplot(m.egY.patient.ratio, x = "Patient", y = "loss.ratio", title = "chrY genes ratio of cells with 0 reads on Y per (male) patient", xlab = "Patient", ylab = "Loss Ratio") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
ggsave("mosaicY/chrY_genes_ratio_per_patient.barplot.pdf")
...
avg.exp.all_but_Y <- as.data.frame(GetAssayData(immune.seurat, slot = "data", assay = "RNA")[,unlist(egY.zero.cells)])
avg.exp.all_but_Y <- avg.exp.all_but_Y[which(!row.names(avg.exp.all_but_Y) %in% egY),]
pdf("mosaicY/not_on_chrY_Genes_avgexp_in_no_chrY_cells.hist.pdf")
...
for(n in 1:1000){
  ...
  # Retrieve expression data per cell
  tmp.egNOTY.expr <- as.data.frame(GetAssayData(immune.seurat, slot = "data", assay = "RNA")[CTRL.boots.genes[[n]],])
...
ggsave("mosaicY/genes_ratio_per_patient_bootstrapped.barplot.pdf")
...
pdf("mosaicY/bootstrap_fit_cutoff.scatter.pdf")
...
ggsave("mosaicY/bootstrap_directionality.barplot.pdf")
...
ggsave("mosaicY/genes_ratio_diff_per_patient_bootstrapped.barplot.pdf")
